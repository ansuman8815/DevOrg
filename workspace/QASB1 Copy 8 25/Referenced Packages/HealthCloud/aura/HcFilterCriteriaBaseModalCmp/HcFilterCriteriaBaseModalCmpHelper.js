({
    ...
    createUpdateFilterList: function(component, event, helper) {
        ...
        var configObject = filterCriteriaCmp.get("v.configObject");

        // columns selection
        var columnSelector = filterCriteriaCmp.find("FilterColumnSelector");
        var columns = columnSelector != null ? columnSelector.get("v.columnFields") : [];
        ...
        action.setCallback(this, function(result) {
            ...
            if (!$A.util.isUndefinedOrNull(res) && state.toUpperCase() === "SUCCESS") {
                ...
                if ($A.util.isUndefinedOrNull(configObject)) {
                    ...
                    var messageBody = {
                        message: $A.get("$Label.HealthCloudGA.Msg_Success_Filter_List_Edit")
                    };
                    self.showToast(component, messageBody, true, 'success');
                    var vfRefreshEvent = $A.get("e.HealthCloudGA:HcFilterCriteriaConfigEvent");
                    vfRefreshEvent.setParams({ "message": "refresh" });
                    vfRefreshEvent.fire();
                }
                ...
                var errorMessage = !$A.util.isUndefinedOrNull(result.getError()[0].pageErrors)?result.getError()[0].pageErrors:[{message:failureMessage}];

                this.handleError(component,errorMessage);
